...
const TicketView = () => {
  ...
  const fetchFlightData = async () => {
    ...
    try {
      ...
      if (data.response && data.response.length > 0) {
        // We found a live flight!
        const flight = data.response[0];
        
        // Map AirLabs status to our UI status
        let uiStatus = 'Scheduled';
        if (flight.status === 'en-route' || flight.status === 'active') uiStatus = 'On Time';
        if (flight.status === 'landed') uiStatus = 'Arrived';
        if (flight.status === 'scheduled') uiStatus = 'Scheduled';
        if (flight.status === 'cancelled') uiStatus = 'Cancelled';
        if (flight.dep_delayed > 15) uiStatus = 'Delayed';

        // Format estimated departure
        let estDep = flight.dep_estimated || flight.dep_time || '21:15';
        // Extract just HH:MM if it's a full date string
        if (estDep.includes(' ')) {
           estDep = estDep.split(' ')[1].substring(0, 5);
        }

        setFlightStatus({
          status: uiStatus,
          gate: flight.dep_gate || 'TBA',
          terminal: flight.dep_terminal || '1',
          estimatedDep: estDep,
        ...
        // No live flight found currently (common for future dates or non-daily flights not flying right now)
        // Fallback to "Scheduled" but indicate update success
        console.log("No live flight SL395 found at this moment.");
        setFlightStatus(prev => ({
          ...prev,
          lastUpdated: updatedTime,
          loading: false,
          isRealData: false, // Indicate we are back to static schedule
          error: "Flight not currently active"
        }));
      }

    } catch (err) {
      console.error("AirLabs fetch error:", err);
      // Fallback to simulated data for demo purposes if API fails (e.g. CORS)
      const now = new Date();
      const updatedTime = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
      ...
      setFlightStatus(prev => ({
        ...
        loading: false,
        isRealData: false,
        error: "Connection failed, using schedule"
      }));
    }
  };
  ...
  useEffect(() => {
    ...
    // Auto-refresh removed as per request - Manual refresh only
  }, []);
         ...
         <div className="bg-[#0F1C20] p-8 text-white relative">
           ...
           <div className="absolute top-6 right-6 flex items-center gap-2 bg-black/30 px-3 py-1 rounded-full backdrop-blur-sm border border-white/10">
              ...
              <span className="text-[10px] font-bold tracking-wider uppercase">
                {flightStatus.isRealData ? 'Live Data' : 'Scheduled'}
              </span>
           </div>
           ...
           <div className="flex justify-between text-xs text-gray-400 mt-1">
             ...
             <div className="text-right">
                ...
                <span className={flightStatus.status === 'Delayed' ? 'text-red-400' : 'text-white'}>
                  {flightStatus.estimatedDep} (+1)
                </span>
             </div>
           </div>

           {/* Decorative Circles for the "tear" effect */}
         ...
         <div className="bg-white p-8 pt-2">
            ...
            <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 flex items-center gap-3 mb-6">
               ...
               <div className="text-xs text-gray-600">
                  ...
                  {flightStatus.error ? `Using schedule (${flightStatus.error})` : 'No Online Check-in available.'}
               </div>
            </div>
            ...
            <div className="h-16 bg-[#FF9F68] rounded-xl flex items-center justify-center overflow-hidden relative">
               ...
               <p className="absolute bottom-1 text-[10px] font-mono tracking-[0.2em] text-[#0F1C20]">
                 SL395-TPE-DMK-{flightStatus.isRealData ? 'LIVE' : 'SCHED'}
               </p>
            </div>

            <div className="mt-4 flex justify-between items-center text-[10px] text-gray-400 font-mono">
               <span>Source: {flightStatus.isRealData ? 'AirLabs (Live)' : 'Schedule'}</span>
